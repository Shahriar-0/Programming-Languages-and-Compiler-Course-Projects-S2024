...
functionDeclaration returns [FunctionDeclaration functionDeclarationRet]:
	...
	def = DEF {$functionDeclarationRet.setLine($def.line);} 
	id = IDENTIFIER {
...
patternMatching returns [PatternDeclaration patternRet]:
	pat = PATTERN {int patternLine = $pat.line;}
	...
	(
		...
		ASSIGN 
		e = expression {$patternRet.addReturnExp($e.expRet);}
	)*
	SEMICOLLON;

main returns [MainDeclaration mainRet]:
	{$mainRet = new MainDeclaration();}
	DEF m = MAIN {$mainRet.setLine($m.line);}
	LPAR 
	RPAR
...
condition returns [ArrayList<Expression> conditionRet]:
	...
	(
		...
		(
			(AND | OR) (LPAR)? 
			c = condition {$conditionRet.addAll($c.conditionRet);}
			(RPAR)?
		)*
	)*
	;
...
accessList returns [Expression accessListExp]:
	LBRACK 
	e = expression {$accessListExp = $e.expRet;} 
	RBRACK
	;
...
additiveExpression returns [Expression expRet]:
	...
	| m2 = multiplicativeExpression {$expRet = $m2.expRet;}
	;
