...
functionDeclaration
	returns[FunctionDeclaration functionDeclarationRet]
	: //TODO:construct functionDeclaration node
	def = DEF id = IDENTIFIER f = functionArgumentsDeclaration b = body END
	;

functionArgumentsDeclaration
	returns[ArrayList<VarDeclaration> argRet]
	: {
...
patternMatching
	returns[PatternDeclaration patternRet]
	: //TODO:cunstruct patterDeclaration node
	pat = PATTERN patternName = IDENTIFIER LPAR targetVar = IDENTIFIER RPAR (
		PATTERN_MATCHING_SEPARATOR c = condition ASSIGN e = expression
	)* SEMICOLLON
	;

main
	returns[MainDeclaration mainRet]
	: {
        $mainRet = new MainDeclaration();
    } DEF m = MAIN {
        $mainRet.setLine($m.line);
    } LPAR RPAR b = body {$mainRet.setBody($b.bodyRet);} END
	;

functionArguments
	returns[ArrayList<Expression> funcArgsRet]
	: {
...
returnStatement
	returns[ReturnStatement returnStmtRet]
	: {
...
ifStatement
	...
	: {
		...
		| LPAR c2 = condition RPAR {$ifRet.addCondition($c2.conditionRet);}
	) b = loopBody {
...
putsStatement
	returns[PutStatement putRet]
	: //TODO:construct putStatement node
	p = PUTS LPAR e = expression RPAR SEMICOLLON
	;

lenStatement
	returns[LenStatement lenRet]
	: //TODO:construct lenStatement node
	l = LEN LPAR e = expression RPAR
	;

pushStatement
	returns[PushStatement pushRet]
	: //TODO:construct pushStatement node
	p = PUSH LPAR e1 = expression COMMA e2 = expression RPAR SEMICOLLON
	;

loopDoStatement
	returns[LoopDoStatement loopDoRet]
	: l1 = LOOP DO l2 = loopBody {
...
loopBody
	returns[ArrayList<Statement> loopStmts, ArrayList<Expression> loopExps, ReturnStatement loopRetStmt]
	: {
...
forStatement
	returns[ForStatement forStRet]
	: //TODO:construct forStatement node
	f = FOR id = IDENTIFIER IN r = range l = loopBody END
	;

range
	returns[ArrayList<Expression> rangeRet]
	: //TODO:store all expressions that a range can have in rangeRet array
	(LPAR e1 = expression DOUBLEDOT e2 = expression RPAR)
	| (LBRACK (e3 = expression (COMMA e4 = expression)*) RBRACK)
	| id = IDENTIFIER
	;


matchPatternStatement
	returns[MatchPatternStatement matchPatRet]
	: //TODO:construct match pattern node
	id = IDENTIFIER DOT m = MATCH LPAR e = expression RPAR
	;

chopStatement
	returns[ChopStatement chopRet]
	...
	;
chompStatement
	returns[ChompStatement chompRet]
...
assignment
	returns[AssignStatement assignRet]
	: {
...
accessList
	returns[Expression accessListExp]
	: LBRACK e = expression {$accessListExp = $e.expRet;} RBRACK
	;

statement
	returns[Statement stmtRet]
...
body
	returns[ArrayList<Statement> bodyRet]
	: {
...
lambdaFunction
	returns[Expression lambdaRet]
	: a = ARROW fd = functionArgumentsDeclaration LBRACE b = body RBRACE {
...
listValue
	returns[ListValue listValRet]
	: l = LBRACK f = functionArguments RBRACK {
