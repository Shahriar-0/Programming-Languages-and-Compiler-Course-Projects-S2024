...
public class TypeChecker extends Visitor<Type> {
	...
	public Type visit(FunctionDeclaration functionDeclaration) {
		if (visited.contains(functionDeclaration.getFunctionName().getName())) {
			try {
				return ((FunctionItem) SymbolTable.root.getItem(
		...
		if (returnStmtsTypes.size() > 1) {
			typeErrors.add(
				new FunctionInconsistentReturnTypes(
			...
			try {
				((FunctionItem) SymbolTable.root.getItem(
					FunctionItem.START_KEY +
					functionDeclaration.getFunctionName().getName())
				).setReturnType(returnStmtsTypes.iterator().next());
			} catch (ItemNotFound e) {}
	...
	public Type visit(PatternDeclaration patternDeclaration) {
		...
		try {
			...
			if (returnStmtsTypes.size() != 1) {
				typeErrors.add(
					new PatternInconsistentReturnTypes(
	...
	public Type visit(AccessExpression accessExpression) {
		if (accessExpression.isFunctionCall()) {
			...
			try {
				FunctionItem functionItem = (FunctionItem) SymbolTable.root.getItem(
					FunctionItem.START_KEY + functionName.getName()
				);
				List<VarDeclaration> defaultVals = functionItem
					.getFunctionDeclaration()
					.getArgs()
					.stream()
					.filter(a -> a.getDefaultVal() != null)
					.toList();
				...
				Type nonFunctionType = accessExpression
					.getAccessedExpression()
					.accept(this);
				if (!(nonFunctionType instanceof FptrType fptrType)) {
					typeErrors.add(
						new IsNotCallable(accessExpression.getLine())
					);
					return new NoType();
				} else {
					try {
						FunctionItem functionItem = (FunctionItem) SymbolTable.root.getItem(
							FunctionItem.START_KEY + fptrType.getFunctionName()
						);
						List<VarDeclaration> defaultVals = functionItem
							.getFunctionDeclaration()
							.getArgs()
		...
		try {
			...
			try {
				var item = (VarItem) SymbolTable.top.getItem(
					VarItem.START_KEY + varItem.getName()
				);
	...
	public Type visit(AssignStatement assignStatement) {
		if (assignStatement.isAccessList()) {
			...
			try {
				...
				try {
					item =
						(VarItem) SymbolTable.top.getItem(
							VarItem.START_KEY + newVarItem.getName()
						);
	...
	public Type visit(ListValue listValue) {
		...
		if (listTypes.size() != 1) {
			typeErrors.add(
				new ListElementsInconsistentType(listValue.getLine())
			);
			return null;
		}
	...
	public Type visit(RangeExpression rangeExpression) {
		...
		if (rangeType.equals(RangeType.IDENTIFIER)) {
			Type rangeIdentifierType = rangeExpression
				.getRangeExpressions()
				.getFirst()
				.accept(this);
			...
			if (typesOfElements.size() != 1) {
				typeErrors.add(
					new ListElementsInconsistentType(rangeExpression.getLine())
				);
				return new NoType();
			}
