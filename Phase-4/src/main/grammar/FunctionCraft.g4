...
functionDeclaration
    ...
    } f = functionArgumentsDeclaration {$functionDeclarationRet.setArgs($f.argRet);} b = body {$functionDeclarationRet.setBody($b.bodyRet);} END
	;
...
main
	...
	: {$mainRet = new MainDeclaration();} DEF m = MAIN {$mainRet.setLine($m.line);} LPAR RPAR b = body {$mainRet.setBody($b.bodyRet);} END
	;
...
returnStatement
	...
	: {$returnStmtRet = new ReturnStatement();} r = RETURN (
		e = expression {$returnStmtRet.setReturnExp($e.expRet);}
...
ifStatement
	...
	) b = loopBody {$ifRet.setThenBody($b.loopStmtsRet);} (
		ELSE b2 = loopBody {$ifRet.setElseBody($b2.loopStmtsRet);}
	)? END
	;
...
loopBody
	returns[ArrayList<Statement> loopStmtsRet]
	: {$loopStmtsRet = new ArrayList<Statement>();} (
		s = statement {$loopStmtsRet.add($s.stmtRet);}
		| BREAK {BreakStatement b = new BreakStatement();} (
			IF c1 = condition {b.setConditions($c1.conditionRet);}
		)? {$loopStmtsRet.add(b);} SEMICOLLON
		| NEXT {NextStatement n = new NextStatement();} (
			IF c2 = condition {n.setConditions($c2.conditionRet);}
...
range
	returns[RangeExpression rangeRet]
	: {
		...
		(
			LPAR e1 = expression DOUBLEDOT e2 = expression {
                exps.add($e1.expRet);
                exps.add($e2.expRet);
		...
		| {rangeType = RangeType.LIST;} (
			LBRACK (
				e3 = expression {
                        exps.add($e3.expRet);
                        line = $LBRACK.line;
                    } (COMMA e4 = expression {exps.add($e4.expRet);})*
			) RBRACK
		)
		| id = IDENTIFIER {
            Identifier id_ = new Identifier($id.text);
...
body
	...
	: {$bodyRet = new ArrayList<Statement>();} (s = statement {$bodyRet.add($s.stmtRet);})* (
